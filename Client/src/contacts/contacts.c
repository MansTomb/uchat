...
static void builder_2(t_info *info, t_contacts *cont) {
    ...
    cont->tree_store = GTK_TREE_STORE(gtk_builder_get_object(cont->builder,
                                                             "contacts_tree"));
    cont->log_render = GTK_CELL_RENDERER(gtk_builder_get_object(cont->builder,
                                                             "cr1"));
    cont->stat_render = GTK_CELL_RENDERER_PIXBUF(gtk_builder_get_object(
                                                        cont->builder, "pr"));
    cont->log_col = GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(cont->builder,
                                                             "login_column"));
    cont->stat_col = GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(cont->builder,
                                                             "status"));
}
...
void mx_contacts_build(t_info *info, t_contacts *cont) {
    ...
    gtk_builder_add_from_file(cont->builder, MX_GLADE_CONT, NULL);
    builder_2(info, cont);
    ...
    gtk_tree_view_column_add_attribute(cont->log_col,
                                                 cont->log_render, "text", 0);
    gtk_tree_view_column_add_attribute(cont->stat_col,
                            GTK_CELL_RENDERER(cont->stat_render), "pixbuf", 1);
    mx_get_json_contacts(info);
    mx_create_table(info, cont);
    gtk_stack_add_titled(GTK_STACK(info->windows->ms->menu_stack),
                                           cont->box, "Contacts", "Contacts");
