...
static void label_settings(GtkWidget *lable) {
    gtk_widget_set_size_request(lable, 860, 1);
    gtk_label_set_line_wrap(GTK_LABEL(lable), TRUE);
    gtk_label_set_selectable(GTK_LABEL(lable), TRUE);
    gtk_label_set_max_width_chars(GTK_LABEL(lable), 100);
}
