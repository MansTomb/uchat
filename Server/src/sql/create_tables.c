...
static int create_triggers(sqlite3 *db) {
    char *query = MX_TRIGGER(add_profile, INSERT)
        "users BEGIN " MX_INSERT(users_profiles, user_id) "(NEW.id); "
        ...
        MX_TRIGGER(del_group, DELETE) "users_groups BEGIN UPDATE contacts_list"
        "s SET group_id = 0 WHERE user_id = OLD.user_id AND group_id = "
        "OLD.group_id; END;";
    char *err = NULL;
    int rc = sqlite3_exec(db, query, NULL, NULL, &err);
