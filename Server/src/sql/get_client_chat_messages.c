...
static int callback(void *data, int argc, char **argv, char **cols) {
    ...
    const char *fields[] = {"uid", "mid", "login", "time", "content"};

    cJSON_AddNumberToObject(contact, fields[0], atoi(argv[0]));
    cJSON_AddNumberToObject(contact, fields[1], atoi(argv[1]));
    for (uint32_t i = 2; i < sizeof(fields) / sizeof(fields[0]); ++i)
...
cJSON *mx_get_client_chat_messages(sqlite3 *db, cJSON *jsn) {
    ...
    asprintf(&query, "SELECT * FROM "
            "(SELECT m.user_id, m.id, u.login, m.send_time, m.content "
            "FROM messages AS m JOIN users AS u ON m.user_id = u.id "
            "WHERE m.chat_id = %i ORDER BY m.send_time DESC LIMIT 20) "
