...
static int get_id(void *data, int argc, char **argv, char **cols) {
    ...
    cJSON_AddStringToObject(data, "cname", argv[1]);
...
static void create_new_chat(sqlite3 *db, cJSON *jsn) {
    ...
    asprintf(&query, "INSERT INTO chats VALUES (NULL, 1, ''); SELECT "
            "last_insert_rowid(), login FROM users WHERE id = %i; INSERT INTO "
            "users_chats VALUES (%i, last_insert_rowid(), 1), "
            "(%i, (SELECT max(id) FROM chats), 1);", MX_VINT(jsn, "uid2"),
